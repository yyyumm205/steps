...
当前增量（2026-09-19）：依据当前APK只有准备流程可操作、T2a/T2b.1尚未接入页面的事实，调整交付顺序为“原生完整演示 → 真实短采与本地保存 → 上传及后端闭环 → 恢复/长时与试用门禁”。新增待实现T2-P与B23，复用正式页面、状态和持久化，在开发构建隔离模拟设备与传输。准备页按E11收口，硬件只读取证及最小安全出口开发可并行推进。总纲、研究数据含义和Q01/Q02决策状态保持不变；本轮只修改四份规格文档，未实现演示或发布新APK。

更新日期：2026-09-19。分支：`codex/free-living-session-spec`。本轮文档核对见validation的E13；最近运行证据仍为0.6.4-t2b（30）的E12：147项JVM、17项Android测试及构建通过。0.6.3-t1（29）的真机只读证据保留为E11。Q01/Q02待确认，Q03已确认。第1–4节保留 `29486e6` 当时的文档审阅依据；第5节起为逐次变更。历史段落按当时版本理解，当前决策以requirements第6节为准。原始资料和本地配置保留。
