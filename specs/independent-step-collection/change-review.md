...
当前增量（2026-09-19）：在[总纲](../../CONSTITUTION.md)2.1.0下修订T2-P入口层级与任务反馈，版本0.6.6-t2p（32），见第11节及validation的E16。完整演示及实际本地保存沿用E15；设备确认、数据内容和回执仍为模拟。Q01/Q02状态保持原义，后续交付继续为真实采集与本地数据保全。
