...
当前基线（2026-09-21，总纲3.8.0）：原版Android 0.5.3是实验设计与设备行为的默认信任参考。所有主动差异按第16节记录原版行为、当前行为、改变依据、数据影响、验证证据和确认状态；Python SDK用于补充协议证据，不替代原版Android行为。离线单一用户名及其研究编号规则见第16节。软件检查与真机结论分别记录。

当前增量（2026-09-24，0.8.17）：修复充电盒内测试暴露的状态与恢复问题。采集中断联或重连待核对时，原页显示“采集状态待确认”并隐藏运行计时，恢复入口保持原位；前3次自动重连间隔3秒，之后60秒持续尝试，手动重连可立即打断退避。下载保留既有有限失败和无进展边界。正式开始前错误码0也主动取得同连接、5秒内的明确未充电回复，START发送前再核对最新电量；`-16`保持原窄兼容。GATT在服务发现或就绪前断开时立即反馈，明确失败不再等待30秒总超时。既有研究数据、参考数字、账本与冻结ZIP保持原契约；本轮测试数量、构建、安装和真机结论由[validation.md](validation.md)统一记录。

当前增量（2026-09-22，E37）：0.8.10-save-recovery（52）为连续3轮无进展下载提供片段保全和显式重试出口，重复DATA只核对内容，有实际进展才刷新超时；旧记录保全的内部检查维持公开保存状态。普通session核对前缀后续传，未知时间旧备份保留片段并新建隔离attempt完整重读。当前用户名过滤与冻结身份沿用既有规则；无owner入口错误清理旧视图，无session错误／恢复页可返回准备设置，活跃owner错误保留当前任务。629项JVM、70项模拟器行为、构建、签名升级和独立审查通过，按需截图目录导出1项跳过；真机复测未执行，详见E37。用户名架构、后端契约及16 KiB常规窗口保持既有范围。

上一增量（2026-09-22，E36）：0.8.9-upload-guard（51）区分发送前可恢复网络失败、云端结果不确定及永久目标拒绝。结果不确定和永久拒绝停止自动重发，保留原ZIP与任务绑定并显示研究者核对；408、429、5xx及发送前连接失败继续有限重试。研究导入器0.2.5拒绝既有目录中的符号链接、非普通文件和超限JSON，索引失败保留上一完整版本；已完整下载但回执尚未落盘的云端文件会保留并明确告警，避免静默遗漏。研究manifest、原始文件及历史冻结包不改写。最终发布回归、签名包哈希与原位升级证据见validation E36。

当前增量（2026-09-21，E35）：0.8.8-ring-defer（50）已恢复原版“暂存到戒指”的数据位置与待传锁。当前仍保留已确认的停止确认后再选择、参考步数先落盘；立即分支下载到手机后后台上传。新`DEFER_ON_RING`和journal v13只用于手机恢复，不进入研究包；历史`SAVE_LATER`不迁移。软件回归、构建、独立审查与三星原位签名升级通过；真实暂存、恢复及设备限制统一见E35。
...
更新日期：2026-09-24。分支：`codex/free-living-session-spec`。工作流文档检查与各版本运行证据见[validation](validation.md)，当前进度见[plan](plan.md)。第1–4节保留 `29486e6` 当时的文档审阅依据；第5节起为逐次变更。历史段落按当时版本理解，当前决策以requirements第6节和本文件第16节为准。原始资料和本地配置保留。
...
| 活动与三种收尾 | 原版[MainActivity:793](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/MainActivity.kt)在STOP前选择立即上传、暂存戒指或删除；后续收集主观评价 | 0.8.8已恢复“暂存到戒指”：参考先保存，不READ、不打包、不上传，人工“下载并上传”前保持待传锁。停止后选择与参考先落盘为已确认差异，软件和设备验证范围见E35 |
...
| 本地账本与参考 | 原版UploadSessionStore保存上传元数据及传输状态；原版停止／下载路径还承担采集结束信息整理 | [FreeLivingSessionStore](../../android/app/src/main/java/com/nexthci/ringfitness/FreeLivingSessionStore.kt)以原子账本保存身份、活动、请求／确认、设备证据、参考、原始文件及传输策略；参考确认先于READ。0.8.8账本v13增加`DEFER_ON_RING`并兼容v12及历史`SAVE_LATER`；研究manifest仍为v2–v7，内部恢复状态不进入研究包 |
...
| 下载与上传 | 原版HealthFlashDownload按8 KiB窗口读取并有限自动补缺，UploadWorker以持久任务对普通错误最多自动尝试5次；SDK[sensors.py:624](../../original/ring-python-sdk-50549cc3/src/ring_python_sdk/session/sensors.py)提供分窗读取接口 | 当前下载仍用16 KiB窗、超时后转手动恢复；完成后最终STATUS/LIST复核、尾部增长续传及跨重开处理已实现。上传对发送前失败及408、429、5xx最多持久重试5次；永久拒绝和POST后回执不明停止重发。0.8.8的`DEFER_ON_RING`在人工操作前完全停留于戒指；历史`SAVE_LATER`仍只等待云盘上传。手动入口仍受BLE权限前置影响 |
...
| H．长时系统保护 | 0.8.8已增加通知权限、电池设置入口与dataSync上传前台保护；自动策略与Manifest检查通过 | 接续验证拒绝、系统回收、完全后台联网恢复、整机重启和长文件；按目标手机系统分别取证 |
...
以上待验方案沿用已确认的分段、清零、唯一参考及三种收尾规则。下一步先恢复原版STOP顺序、手机边界和陌生旧记录非阻塞开始，再真机验证已实现的持续重连，并补齐断线审计、延迟记录、8 KiB补缺、统一TIME证据和卸载前保全；随后完成设备故障恢复、逐级长时及独立操作验收。意外停止的异常出口与下载末尾复核继续接受完整自动测试和真机验证。

## 16. 原版信任基线差异登记（总纲 3.8.0）
...
| 设备详情 | 原版[MainActivity:340–343](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/MainActivity.kt)显示名称、RSSI、电量和固件，并可刷新电量 | 当前正式入口只展示保存的名称及连接／就绪状态；0.8.17正式owner为开始预检主动查询电量与充电状态，常规RSSI、电量、固件、完整地址和Flash计数尚未形成完整可用详情。`RingPreparationController`中的相关实现当前不可达 | 原版四项为**应恢复**；完整地址与Flash计数为**已确认的技术补充** | 电量影响长时采集，固件、地址和记录量影响追溯。详情按需展开，不增加主路径步骤；充电预检不等同于完整设备详情，后续展示须验证数据来自当前连接 |
...
| START时序与错误处理 | 空闲后约500 ms发START，约1000 ms首查，之后约1500 ms轮询；每次最多3查、最多2次START；非零错误仍进入有限尝试 | 0.8.17沿用校时后的同连接完整观测，保留500／1000／1500 ms和最多3轮确认，只发一次START；普通错误码0也须主动取得新鲜未充电回复，发送前再次核对。严格的同连接charging `-16`恢复证据继续执行独立完整预检，其他空闲错误阻止开始 | 单次START及较严格门禁仍为**证据不足**；充电预检为本轮盒内测试问题的**技术修复** | START等待中重新入盒、旧连接或过期电量回复均须阻止发送；成功确认继续要求错误码0和对应新记录。软件及真机证据统一见本轮validation，不以充电预检代替实际开始确认 |
| 正式开始的充电状态 | 原版电量查询按用户请求或恢复探测调度，常规START预检没有同等的每次新鲜未充电证据门禁 | 0.8.17在就绪及校时后开始检查主动查询BATTERY，要求同连接、5秒内且明确未充电；实际发送START前重新验证最新电量。错误码0与`-16`分支都受该末端检查约束 | **相对原版的技术增强**：修复用户报告的盒内连接与采集状态混淆，设备覆盖范围按实测记录 | 未知充电状态保持待确认并提供原位重试；旧固件回复是否具有充电字段须列入兼容验证。`-16`允许条件、研究manifest和已保存实验数据均保持原契约 |
...
| 初次BLE连接超时 | 原版BLE客户端及采集服务没有同等的整段初连超时；GATT断开由客户端断开处理分支报告 | 0.8.17由[RingBleClient](../../android/app/src/main/java/com/nexthci/ringfitness/RingBleClient.kt)向正式owner明确报告GATT断开，覆盖服务发现或ready之前的失败；没有终止回调且30秒仍未ready时，再由[RealCollectionController](../../android/app/src/main/java/com/nexthci/ringfitness/RealCollectionController.kt)兜底超时 | 早期断开即时反馈为**技术修复**；30秒兜底阈值仍为**证据不足** | 覆盖就绪前断开、迟到旧回调、就绪后断开及无回调超时；已明确终止的连接及时进入恢复，失败回调不重复创建重连任务。厂商栈覆盖按实机证据记录 |
| 连接就绪后的查询节奏 | 原版[RingCaptureService:1565–1581](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingCaptureService.kt)在普通空闲连接就绪后约250ms查询STATUS、约600ms查询固件；电量查询按用户请求或恢复探测另行调度 | 当前[RealCollectionService](../../android/app/src/main/java/com/nexthci/ringfitness/RealCollectionService.kt)在BLE ready后立即交给正式owner查询STATUS/LIST；0.8.17增加开始前BATTERY查询。旧准备控制器不属于正式运行路径 | 立即STATUS/LIST节奏为**证据不足**；充电查询按本轮技术修复单独登记 | 固件详情查询仍待恢复。用同一手机／戒指核对初始化与查询节奏，记录完整TX、回复与文件指纹；新增充电门禁须验证正常离盒流程不会误阻塞 |
...
| STOP与Flash收尾 | 原版STOP后约500ms查STATUS；仍采集则每约1000ms继续查；首次stopped后等待约5000ms，再查询LIST并等待迟到记录 | 0.8.15保留500ms首查；首次完整STATUS/LIST稳定后间隔1秒取得第二份完整快照，两份一致才收尾；持续collecting、Flash增长和LIST迟到均有限退出。已受理STOP后的进程重开继续只读查询，不重复发送STOP | **软件已实现，属于相对原版的明确时序调整** | 663项JVM与98项instrumentation覆盖持续collecting、LIST迟到、计数增长、已受理STOP重开和最终定稿；须用真机确认实际回调节奏、Flash尾部、断连和整机恢复后的耗时 |
| 收尾选择时点 | 原版在发送STOP前选择立即上传、暂存戒指、删除或继续，STOP后按既定选择处理 | 先完成STOP确认，再显示“保存并上传／暂存到戒指／放弃本段”；两个保留分支都先保存计步器数字与选择 | **已确认改变**：总纲3.8.0保留当前停止后选择和参考先落盘 | 参考步数属于本研究新增字段，先落盘降低下载前丢失风险；STOP后的页面必须恢复同一草稿和选择。E35验证断连、重开、重复提交及写失败，不把原版STOP前顺序误记为相同行为 |
| 暂存数据位置 | 原版[MainActivity:797–798](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/MainActivity.kt)区分`UPLOAD_NOW`与`DEFER`；[RingCaptureService:1154–1163](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingCaptureService.kt)在DEFER时保留Flash记录、不发READ，后续入口为“下载到手机并上传”。待传任务阻止下一次采集 | 0.8.7的`SAVE_LATER`已先下载到手机，只暂停云盘上传。0.8.8新增`DEFER_ON_RING`：不READ、不打包、不上传，冷启／重启／联网均保持人工等待；手机备份前阻止下一段及身份／位置／戒指切换。历史`SAVE_LATER`保持旧义 | 原版数据位置与待传锁**已恢复，软件与页面回归通过**；参考先落盘为已确认增强，真机范围见E35 | 原始数据尚只在戒指时存在覆盖、换环和归属风险，界面明确数据位置与恢复动作。“下载并上传”核对原戒指与记录锚后才READ；首页和记录列表的放弃均保留二次确认和排除标记。journal v13内部状态不进入研究包 |
| 参考步数与无法提供读数 | 原版Android 0.5.3无计步器字段；步数采集交接说明要求一个session对应一个整数，并允许上传前修改 | 3.7.0按用户新要求将被试入口收敛为填写数字（含0）或放弃，去掉质量选择和原因输入；未尝试上传前直接修改数字，保留原值审计并原子失效旧ZIP | 新入口规则为**已确认改变**；上传前纠错为**应恢复，软件已实现** | 历史missing/unreliable不迁移。STOP未确认时可先保留数字，继续标记unreliable并由系统说明停止未知原因；以后确认停止也不自动提升质量。修改既有unreliable仅改数字，保留原状态／原因；上传取得发布权后冻结，结果以可靠保存为准。当前验收见E34 |
| 独立session异常说明 | 原版结束时可填写本段活动细节；完整主观评分另属已退出范围 | 3.7.0已移除被试读数原因输入；独立表达误混其他活动、额外休息、佩戴中断等本段事件的入口仍待补齐 | 窄范围session异常说明为**应恢复**；原版评分量表退出仍是**已确认改变** | 活动／佩戴事件应独立于计步器质量保存。后续补简短可选说明并写入session质量与研究包，验证空白、重开、修订和后端读回 |
...
| 下载窗口与补缺 | 原版[RingCaptureService:2138、2140](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingCaptureService.kt)使用8 KiB READ窗口，并对缺失窗口作最多3次自动补读 | 当前[RealCollectionController](../../android/app/src/main/java/com/nexthci/ringfitness/RealCollectionController.kt)保持16 KiB常规窗口；0.8.10连续3轮无进展后保留断点、停止自动读取并转显式重试，重复DATA不续超时 | 窗口及补缺仍为**应恢复**；E37无进展出口的软件及模拟器验证通过，真机待验 | E37覆盖有限退出、前缀重读、迟到回调及手动恢复，常规窗口保持现状。8 KiB及有限补缺继续按原版差异单独取证；真实断线、缺窗和长记录待设备验证 |
...
| 连接与就绪语义 | 原版将BLE连接和可执行采集状态分阶段呈现，操作入口由实际状态决定 | 0.8.17只有当前连接有效且同一session确认采集时显示“正在采集”和运行计时；断联或重连待核对时原页显示“采集状态待确认”，保留开始日期和任务，隐藏运行计时。恢复入口原位更新“连接中…”或“重新连接” | **应恢复，针对本轮界面缺陷修复** | 验证首页、采集页和前台通知一致；自动连接、失败及手动恢复过程中控件位置稳定，停止入口只在真实状态允许时恢复。手机计时不作为离线信号覆盖证据 |
...
| 记录列表与本地工具 | 原版[MainActivity:482、1055–1128](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/MainActivity.kt)提供完整列表、戒指待传的“下载到手机并上传”、云盘重传、删除本地记录和CSV导出 | 当前记录页展示该用户全部本地完整记录；普通上传失败可重试，历史`SAVE_LATER`可手动上传。0.8.8须让戒指暂存任务显示“下载并上传”与放弃，并保留其他记录。完整性校验异常继续隔离；导出与无BLE权限独立上传仍待补 | 戒指待传入口为**应恢复，待E35验证**；查看全部已恢复；导出及分类恢复为**应恢复**；删除本地记录为**证据不足** | BLE仅是戒指待下载动作的前置，本地记录查看和云盘重传不应被BLE权限阻断。E35覆盖两种手动动作不串线；删除须明确原始数据保全、云端回执及其他记录隔离后再开放 |
...
| 正式包中的旧服务 | 原版只使用`RingCaptureService`和`UploadJobService`承担旧采集／上传契约 | 0.8.9正式Manifest仅注册`RealCollectionService`和`RealUploadService`；旧Activity和旧服务类保留为源码参考，包管理器无法解析旧服务 | **应恢复，软件已实现**：正式独立版只有一套采集与上传owner | Release交付测试断言两项旧服务均未注册；原位升级还需核对旧JobScheduler任务随组件注销后不再运行，现行任务与文件保持 |
...
| 上传回执校验 | 原版读取响应数组第一项的名称和ID | 当前要求响应恰有一项、文件ID格式有效、服务端size与冻结ZIP字节数完全一致；POST后响应缺失或不可信时进入研究者核对，客户端不重发 | **已确认的完整性增强** | 严格回执阻止错误文件被记为成功；须用真实云盘覆盖正常回执、多项／缺字段／大小不符及POST中断。核对态保留原ZIP和任务绑定，后端去重不替代客户端边界判断 |
...
| 普通上传失败重试 | 0.5.3把普通上传异常重新置为queued，交给持久JobScheduler指数退避，最多自动尝试5次；永久错误或达到上限后才进入failed | 当前[RealUploadQueue](../../android/app/src/main/java/com/nexthci/ringfitness/RealUploadQueue.kt)对发送前失败及408、429、5xx最多自动尝试5次；永久目标拒绝与POST后回执不明立即停止并保留核对态。历史`SAVE_LATER`等待用户首次手动触发；新`DEFER_ON_RING`在手机备份前不得创建上传任务 | 明确可恢复失败重试为**应恢复，软件已实现**；结果不明停止重发为**数据完整性增强**；戒指暂存隔离已由E35实现 | 队列测试覆盖五次上限、手动续试、持久回执恢复、结果不明和永久拒绝；E35覆盖待下载隔离。手机重启、真实弱网和POST中断仍待验证 |
...
| 采集中重连与断线审计 | 原版[RingBleClient:544、557](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingBleClient.kt)连接丢失后约每3秒持续重连，[RingCaptureService:1517](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingCaptureService.kt)记录断线区间 | 0.8.17保留未完成采集的长期自动恢复，前3次间隔3秒，之后60秒持续尝试；手动连接立即生效，同一失效连接只排一个计时器，新连接或owner关闭使旧任务失效。下载继续按既有失败和无进展上限退出。断线区间尚未完整进入session质量证据 | 持续恢复为**应保持的基线**；60秒退避为减少反复连接的**明确技术调整**；断线审计仍为**应恢复** | 验证超过3次失败后仍能恢复同一session、手动重连不与旧定时器并行、核对成功后重置快速重连、下载有限退出及文件不变。真实远离返回、锁屏后台、进程／整机重启和断线时段落盘继续按设备矩阵验收 |
...
| 后台权限与系统限制引导 | 原版[MainActivity:1204–1217](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/MainActivity.kt)在权限检查时合并请求蓝牙／定位和通知权限；未取得电池优化豁免时主动打开系统豁免确认。采集服务持有前台通知与唤醒锁，上传任务执行时显示前台通知 | 当前API 33+只在首次进入采集前请求一次通知权限；拒绝后仍可进入采集，准备页仅在通知关闭或电池优化仍生效时显示“开启通知／允许后台运行”，由用户主动进入系统设置。采集服务保留前台通知与唤醒锁，上传服务已补`dataSync`前台通知 | 设置入口收敛属于**界面差异，证据不足**；前台采集与上传保护为**应恢复，软件已实现、待设备验证** | 通知拒绝不改变戒指采集协议，但可能降低后台状态可见性；电池限制可能中断重连、下载或上传。分别验证首次允许、首次拒绝、设置后恢复、锁屏、切后台、网络切换、进程／整机重启及数小时运行；华为与三星单列，不用短时前台成功替代长时证据 |
...
当前实施顺序由上述分类约束：先以本轮模拟器证据和真机STOP复测收口现有工作区，再补手机边界入包及陌生旧记录非阻塞开始；正式交付前隔离旧采集／上传服务。随后修复自助恢复入口、设备详情、断线审计、通用下载记录迟到、8 KiB补缺、研究端设备索引和卸载前保全，并在真机验证已实现的持续重连。START次数、`-16`兼容、陌生正在采集任务、查询超时、WRITE_NO_RESPONSE及非零回调、MTU门控、GATT线程／PHY、INFO/TIME解析和复杂记录归属保留原始数据后做两手机真机对照；冻结ZIP保留期依据长时空间证据确定。每次改变原版行为时，同步更新本表、requirements验收项和validation证据。

### E38 恢复兼容与有限下载（2026-09-22）

本轮确认并记录三类实现差异：下载停滞属于代码进展判断缺失，已统一使用跨连接无进展预算；保护停止入口曾可能由宽松候选触发，已收窄到既有持久化保全契约；普通后台Job提升前台服务可能被系统拒绝，已降级继续处理持久队列。上述改动保持原始文件、正式开始／停止证据和结果不明时不重复上传的门槛。软件回归与模拟器验证见validation E38；华为／小米、真实戒指、长时和整机恢复仍需设备证据。
