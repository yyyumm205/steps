...
| 下载与上传 | 原版HealthFlashDownload按8 KiB窗口读取并有限自动补缺，UploadWorker以持久任务对普通错误最多自动尝试5次；SDK[sensors.py:624](../../original/ring-python-sdk-50549cc3/src/ring_python_sdk/session/sensors.py)提供分窗读取接口 | 当前下载仍用16 KiB窗、超时后转手动恢复；完成后最终STATUS/LIST复核、尾部增长续传及跨重开处理已实现。上传已恢复最多5次持久自动重试。0.8.8的`DEFER_ON_RING`须在人工操作前完全停留于戒指；历史`SAVE_LATER`仍只等待云盘上传。手动入口仍受BLE权限前置影响 |
