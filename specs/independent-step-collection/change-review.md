...
更新日期：2026-09-22。分支：`codex/free-living-session-spec`。工作流文档检查与各版本运行证据见[validation](validation.md)，当前进度见[plan](plan.md)。第1–4节保留 `29486e6` 当时的文档审阅依据；第5节起为逐次变更。历史段落按当时版本理解，当前决策以requirements第6节和本文件第16节为准。原始资料和本地配置保留。
...
| START时序与错误处理 | 空闲后约500 ms发START，约1000 ms首查，之后约1500 ms轮询；每次最多3查、最多2次START；非零错误仍进入有限尝试 | 0.8.15复用校时后的同连接完整观测，观测须在30秒内且错误码为0，随后保留500／1000／1500 ms和最多3轮确认；只发一次START。严格的同连接charging `-16`恢复证据继续执行独立完整预检，其他空闲错误阻止开始 | **证据不足**：单次START和更严格门禁保护归属，真实设备恢复能力仍需验证 | 用真实戒指验证正常开始、迟到确认、charging `-16`和断连恢复；保持单次START及有限复查 |
...
| 采集中重连与断线审计 | 原版[RingBleClient:544、557](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingBleClient.kt)连接丢失后约每3秒持续重连，[RingCaptureService:1517](../../original/RingFitness-0.5.3-Android-source/app/src/main/java/com/nexthci/ringfitness/RingCaptureService.kt)记录断线区间 | 当前存在未完成session时约每3秒持续重连；同一失效连接只排一个计时器，手动连接或owner关闭会使旧任务失效。断线区间尚未完整进入session质量证据 | 持续重连为**应恢复，软件已实现**；断线审计仍为**应恢复** | 单元测试已覆盖多轮失败、即时连接失败、迟到回调、手动竞争和任务结束后停止尝试；真机远离、返回、锁屏后台、进程／整机重启及断线时段落盘仍待验证 |
