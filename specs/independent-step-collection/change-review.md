...
当前增量（2026-09-19）：T1b依据E11收口，保留已验证准备路径；补齐同一画板的完整低保真流程、失败恢复和最小视觉规则。独立审阅后明确首页优先恢复原任务、停止不明先保全异常读数，以及仅在原始数据完整落手机后允许新session。T2b.1先实现纯Kotlin请求协调与测试，服务、真实入口、参考值和下载按原计划接续。总纲及Q01/Q02决策状态保持不变；新验收B21/B22分别记录设计和软件证据。
