...
当前增量（2026-09-22，E36）：0.8.9-upload-guard（51）区分发送前可恢复网络失败、云端结果不确定及永久目标拒绝。结果不确定和永久拒绝停止自动重发，保留原ZIP与任务绑定并显示研究者核对；408、429、5xx及发送前连接失败继续有限重试。研究导入器0.2.5拒绝既有目录中的符号链接、非普通文件和超限JSON，索引失败保留上一完整版本；已完整下载但回执尚未落盘的云端文件会保留并明确告警，避免静默遗漏。研究manifest、原始文件及历史冻结包不改写。最终发布回归、签名包哈希与原位升级证据见validation E36。
