...
当前基线（2026-09-21，总纲3.5.0）：原版Android 0.5.3是实验设计与设备行为的默认信任参考。所有主动差异按第16节记录原版行为、当前行为、改变依据、数据影响、验证证据和确认状态；Python SDK用于补充协议证据，不替代原版Android行为。单一用户名字段已确认；编号架构复核及待确认的简化方案见第16节。软件检查与真机结论分别记录。
...
用户名架构复核（2026-09-21）：原版 `MainActivity.submitUsername` 调用远端注册或登录，`RingFitnessApi` 从响应读取 `username` 与 `participant_id`，`ParticipantProfileStore` 只保存当前用户和安装标识；Android 端没有独立的历史用户名映射文件。当前研究端 `importer.py` 与 `health_raw_v2.py` 已输出 `participant_id`，以此区分被试。新增本机随机编号及映射文件方案已停止；单一用户名字段已确认，直接以规范化用户名作为新研究数据编号的方案须确认后实施。该方案可离线登记并跨安装保持同名编号，需要研究者为不同被试分配不同用户名；原有档案及冻结研究包保留原编号。

分类含义如下：**应恢复**表示基线要求仍适用但当前尚未对齐；**已确认改变**表示项目负责人或总纲已明确批准；**证据不足**表示当前实现可能合理，但设备或研究证据尚不足以替代原版行为。

| 范围 | 原版／交接行为 | 当前行为 | 分类与改变依据 | 数据影响、证据及下一步 |
| --- | --- | --- | --- | --- |
| 身份 | 0.5.3通过远端注册／登录取得研究用户，只显示3–24位字母数字用户名；服务端返回`participant_id`，可跨手机保持一致，空闲时可切换用户 | 单一用户名页面已实现；新增随机ID及本机映射方案撤下，编号规则待确认。当前未交付工作区暂沿用既有本地名称路径 | 单一用户名为**已确认改变**；直接使用规范化用户名作为数据编号为**待确认改变**，不宣称与原版服务器架构相同 | 旧研究编号切换出去再回来可能被本地名称路径重分配编号，这是交付前必须修复的已知问题。依据已确认规则复用现有存储后，验证跨用户、退出重登、跨安装及历史归属；冻结包不改写 |
