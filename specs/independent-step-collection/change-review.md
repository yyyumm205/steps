...
当前基线（2026-09-21，总纲3.4.0）：原版Android 0.5.3是实验设计与设备行为的默认信任参考。所有主动差异按第16节记录原版行为、当前行为、改变依据、数据影响、验证证据和“应恢复／已确认改变／证据不足”分类；Python SDK用于补充协议证据，不替代原版Android行为。当前工作区的STOP时序、下载末尾复核、设置owner门禁、五次上传重试及旧记录继续使用已进入软件实现，完整自动测试和真机结论分别记录。
